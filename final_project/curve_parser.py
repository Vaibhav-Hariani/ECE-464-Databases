##Sourced from the pyparser docs
## I would've written my own, but this is so well done that it seems to be unnecessary
##With that said, support for my satistical values (mean, stddev, range, max, min, is my own)
...
def BNF():
    ...
    if not bnf:
        ...
        atom = (
            addop[...]
            + (
                (fn_call | pi | e | x | fnumber | ident).setParseAction(push_first)
