...
class Book:
    ...
    def __repr__(self):
        return (
            f"Book(Name: '{self.name}', Type: '{self.type}', "
...
def book_extract(category) -> Book:
    ...
    upc = driver.find_element(
        By.XPATH,
        "//table[contains(@class, 'table-striped')]//th[text()='UPC']/following-sibling::td",
    ).text
...
def scrape_page(driver: webdriver.Firefox, category) -> list[Book]:
    ...
    book_links = [
        el.find_element(By.CSS_SELECTOR, "h3 a").get_attribute("href")
        for el in book_pages
    ]
...
if __name__ == "__main__":
    ...
    category_list_element = driver.find_element(
        By.CSS_SELECTOR, "div.side_categories ul.nav-list > li > ul"
    )
